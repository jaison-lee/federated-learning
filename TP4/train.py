...
def init_clients(args_):
    ...
    if args_.prop > 0.0:
        num_mal_clients = int(len(os.listdir(data_dir)) * args_.prop)
        mal_client_ids = random.sample(
            range(len(os.listdir(data_dir))), num_mal_clients
        )
    else:
        mal_client_ids = []

    for client_dir in os.listdir(data_dir):
        if os.path.isdir(os.path.join(data_dir, client_dir)) and client_dir.startswith(
            "client_"
        ):
            ...
            client = init_client(
                ...
                logger=logger,
                mal_client=client_id in mal_client_ids,
            )
