...
class Learner:
    ...
    def __init__(
        self, model, criterion, metric, device, optimizer, is_binary_classification
    ):
    ...
    def set_param_tensor(self, param_tensor):
        ...
        for param in self.model.parameters():
            ...
            current_dimension = param.data.view(
                -1,
            ).shape[0]

            param.data = param_tensor[
                current_index : current_index + current_dimension
            ].reshape(param_shape)
    ...
    def set_grad_tensor(self, grad_tensor):
        ...
        for param in self.model.parameters():
            ...
            param.grad.data = deepcopy(
                grad_tensor[current_index : current_index + current_dimension].reshape(
                    param_shape
                )
            )
...
class Label_Flipping_Learner(Learner):
    def __init__(
        ...
        super().__init__(
            model, criterion, metric, device, optimizer, is_binary_classification
        )
