...
def parse_arguments(args_list=None):
    ...
    parser.add_argument(
        "--n_clients", help="number of clients", required=True, type=int
    )
    parser.add_argument(
        "--iid", help="if selected, data are split iid", action="store_true"
    )
    parser.add_argument(
        "--frac",
...
if __name__ == "__main__":
    ...
    if "save_dir" in args_:
        ...
        warnings.warn(
            f"'--save_dir' is not specified, results are saved to {save_dir}!",
            RuntimeWarning,
        )

    os.makedirs(save_dir, exist_ok=True)

    if args_.dataset_name == "mnist" or args_.dataset_name == "cifar10":
        if args_.iid:
            split_type = "iid"
        else:
